...
static void
load_vertex_arrays(struct iqm_model *model, unsigned char *data, int ofs_va, int num_va, int num_v)
{
	int i;
...
static void
load_triangles(struct iqm_model *model, unsigned char *data, int ofs_tris, int num_tris)
{
...
static int
load_material(struct iqm_model *model, char *name)
{
	char buf[256], *p;
...
static void
load_meshes(struct iqm_model *model, unsigned char *data, int ofs_text, int ofs_meshes, int num_meshes)
{
	int i;
	model->num_meshes = num_meshes;
	model->meshes = malloc(num_meshes * sizeof(struct iqm_mesh));
...
static void read_pose(struct iqm_pose *pose, unsigned char *data)
{
...
static void
load_bones(struct iqm_model *model, unsigned char *data, int ofs_text, int ofs_bones, int num_bones)
{
	int i;
	model->num_bones = num_bones;
	model->bones = malloc(num_bones * sizeof(struct iqm_bone));
...
static void
load_anims(struct iqm_model *model, unsigned char *data, int ofs_text, int ofs_anims, int num_anims)
{
	int i;
	model->num_anims = num_anims;
	model->anims = malloc(num_anims * sizeof(struct iqm_anim));
	for (i = 0; i < num_anims; i++) {
		struct iqm_anim *anim = model->anims + i;
...
static void
load_frames(struct iqm_model *model, unsigned char *data,
	int ofs_poses, int ofs_frames, int ofs_bounds, int num_frames, int num_framechannels)
{
	...
	model->bounds = malloc(num_frames * num_bones * sizeof(struct iqm_bounds));
	model->poses = malloc(num_frames * sizeof(struct iqm_pose*));
	...
	for (i = 0; i < num_frames; i++) {
		model->poses[i] = malloc(num_bones * sizeof(struct iqm_pose));
		for (k = 0; k < num_bones; k++) {
			struct chan *chan = chans + k;
			struct iqm_pose *pose = model->poses[i] + k;
			for (n = 0; n < 3; n++) {
...
char *
get_iqm_animation_name(struct iqm_model *model, int anim)
{
...
void
animate_iqm_model(struct iqm_model *model, int anim, int frame, float t)
{
...
void
draw_iqm_instances(struct iqm_model *model, float *trafo, int count)
{
	int i, k, n;
...
void
draw_iqm_model(struct iqm_model *model)
{
	float transform[16];
...
void
draw_iqm_bones(struct iqm_model *model)
{
	int i;
	...
	for (i = 0; i < model->num_bones; i++) {
		struct iqm_bone *bone = model->bones + i;
		if (bone->parent >= 0) {
			struct iqm_bone *pb = model->bones + bone->parent;
			glColor3f(0, 1, 0);
