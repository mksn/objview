...
static void calc_inv_matrix(struct ov_skeleton *s)
{
	int i;
	for (i = 0; i < s->num_bones; i++)
    mat_invert(s->bones[i].inv_bind_matrix,
               s->bones[i].bind_matrix);
}

static void calc_abs_matrix(struct ov_skeleton *s)
{
	int i;
	for (i = 0; i <s->num_bones; i++)
		if (s->bones[i].parent >= 0)
			mat_mul(s->bones[i].abs_bind_matrix,
              s->bones[s->bones[i].parent].abs_bind_matrix,
              s->bones[i].bind_matrix);
		else
			mat_copy(s->bones[i].abs_bind_matrix,
               s->bones[i].bind_matrix);
}

static void calc_matrix_from_pose(struct ov_skeleton *s)
{
	int i;
	for (i = 0; i < s->num_bones; i++)
    mat_from_pose(s->bones[i].bind_matrix,
...
unsigned int loadmaterial(char *material)
{
  ...
  if (s) material = s + 1;
  sprintf(filename, "%s/%s.png", basedir, material);
  texture = load_texture(0, filename);
  if (texture)
    return texture;
  sprintf(filename, "%s/textures/%s.png", basedir, material);
  texture = loadtexture(filename);
...
static void pushpart(struct ov_model *m,
                     struct partarray *a,
                     int first,
                     int last,
                     int material)
{
	...
	a->len++;
  // store the meshes in the model too
}

static struct ov_pose *pushframe(struct ov_unit *du,
                                 int bone_count,
                                 struct ov_pose *pose,
                                 int pose_count);

static void push_skeleton(struct ov_unit *du,
                          struct ov_pose *pose,
                          int pose_count)
{
  int i = 0;
  for (; i < pose_count; i++) {
    du->model->skeleton->bones[i].bind_pose.translate[0] = pose[i].translate[0];
    du->model->skeleton->bones[i].bind_pose.translate[0] = pose[i].translate[1];
    du->model->skeleton->bones[i].bind_pose.translate[0] = pose[i].translate[2];
    du->model->skeleton->bones[i].bind_pose.scale[0] = pose[i].scale[0];
    du->model->skeleton->bones[i].bind_pose.scale[0] = pose[i].scale[1];
    du->model->skeleton->bones[i].bind_pose.scale[0] = pose[i].scale[2];
    du->model->skeleton->bones[i].bind_pose.scale[0] = pose[i].scale[3];
    du->model->skeleton->bones[i].bind_pose.rotate[0] = pose[i].rotate[0];
    du->model->skeleton->bones[i].bind_pose.rotate[0] = pose[i].rotate[1];
    du->model->skeleton->bones[i].bind_pose.rotate[0] = pose[i].rotate[2];
  }
}

static struct ov_animation *pushanim(struct ov_unit *du,
                                     char *name,
                                     struct ov_pose *pose,
                                     int pose_count)
{
	//struct ov_animation *anim;
  int i = du->animations->num_anims;
  if (du->animations->anims != NULL) {
    push_skeleton(du, pose, pose_count);
  } else {
    pushframe(du, du->model->skeleton->num_bones, pose, pose_count);
  }
  du->animations->anims = realloc(du->animations->anims, sizeof(struct ov_animation *)*i);
  du->animations->anims[du->animations->num_anims++] = malloc(sizeof(struct ov_animation));
  memset(du->animations->anims[i], 0, sizeof(struct ov_animation));
	du->animations->anims[i]->name = strdup(name);
	return du->animations->anims[i];
}
...
static struct ov_pose *pushframe(struct ov_unit *du,
                                 ...
                                 int pose_count)
{
  struct ov_animation *anim = du->animations->anims[du->animations->num_anims-1];
  int i = anim->num_frames;
  memcpy(anim->poses[i-1], pose, pose_count);
  anim->poses = realloc(anim->poses, sizeof(struct ov_pose *)*anim->num_frames++);
  anim->poses[i] = malloc(sizeof(struct ov_pose) * MAXBONE);
  memset(anim->poses[i], 0, MAXBONE);
	return anim->poses[i];
}
...
struct ov_model *model_iqe_load_model(char *filename)
{
	...
	while (1) {
		...
		if (s[0] == 'v' && s[1] != 0 && s[2] == 0) {
			...
			default:
				fprintf(stderr, "%s: uh-oh, stmt does not compute!\n", __func__);
				break;
			}
		}
			...
			if (model->skeleton->num_bones < MAXBONE) {
				model->skeleton->bones[model->skeleton->num_bones].name = strdup(parsestring(&sp));
				model->skeleton->bones[model->skeleton->num_bones].parent = parseint(&sp, -1);
				model->skeleton->num_bones++;
			}
		}
		...
		/*
			...
			s = parsestring(&sp);
			(void)pushanim(du, s, pose, pose_count);
		}

		else if (!strcmp(s, "frame")) {
			pose = pushframe(du,
					du->model->skeleton->num_bones,
					pose,
					pose_count);
			pose_count = 0;
		}
		*/
			...
			if (element.len > first)
				pushpart(model, &partbuf, first, element.len, material);
			first = element.len;
			fm = position.len / 3;
		}
	...
	if (model->skeleton->num_bones > 0) {
		calc_matrix_from_pose(model->skeleton);
		calc_abs_matrix(model->skeleton);
		calc_inv_matrix(model->skeleton);
	}

	model->num_triangles = position.len;
	model->num_vertices = model->num_triangles/ 3;
	model->pos = duparray(position.data, position.len, sizeof(float));
	model->norm = duparray(normal.data, normal.len, sizeof(float));
	model->texcoord = duparray(texcoord.data, texcoord.len, sizeof(float));
	//mesh->color = duparray(color.data, color.len, sizeof(float));
	model->blend_index = duparray(blendindex.data, blendindex.len, sizeof(int));
	model->blend_weight = duparray(blendweight.data, blendweight.len, sizeof(float));
	//mesh->aposition = NULL;
	//mesh->anormal = NULL;
...
struct ov_animation *model_iqe_load_animation(struct ov_unit *du, char *filename)
{
	//static mat4 loc_bind_matrix[MAXBONE];
	...
	struct ov_animation *animation = malloc(sizeof *animation);
	animation->skeleton = du->model->skeleton;

	int                  pose_count = 0;
	struct ov_pose       *pose;
	struct ov_bone       *bones;

	fprintf(stderr, "loading iqe animation '%s'\n", filename);

	animation->skeleton->num_bones = 0;
	bones = animation->skeleton->bones;
...
normal.len = 0;
	...
	while (1) {
		...
		if (s[0] == 'v' && s[1] != 0 && s[2] == 0) {
			...
			if (animation->skeleton->num_bones < MAXBONE) {
				animation->skeleton->bones[animation->skeleton->num_bones].name = strdup(parsestring(&sp));
				animation->skeleton->bones[animation->skeleton->num_bones].parent = parseint(&sp, -1);
				animation->skeleton->num_bones++;
			}
		}
    ...
    /*
			...
			if (element.len > first)
				pushpart(du, &partbuf, first, element.len, material);
			first = element.len;
			fm = position.len / 3;
		}
    */
  ...
  */
	...
	if (animation->skeleton->num_bones > 0) {
		calc_matrix_from_pose(animation->skeleton);
		calc_abs_matrix(animation->skeleton);
		calc_inv_matrix(animation->skeleton);
	}

	//animation->num_triangles = position.len;
	//animation->num_vertices = du->animation->num_triangles/ 3;
	//animation->pos = duparray(position.data, position.len, sizeof(float));
	//animation->norm = duparray(normal.data, normal.len, sizeof(float));
	//animation->texcoord = duparray(texcoord.data, texcoord.len, sizeof(float));
	//mesh->color = duparray(color.data, color.len, sizeof(float));
	//animation->blend_index = duparray(blendindex.data, blendindex.len, sizeof(int));
	//animation->blend_weight = duparray(blendweight.data, blendweight.len, sizeof(float));
	//mesh->aposition = NULL;
	//mesh->anormal = NULL;
...
static int haschildren(int *parent, int count, int x)
{
  ...
  */
  ...
  /*
void drawskeleton(struct ov_unit *du)
{
    struct ov_skeleton *skel = du->model->skeleton;
    vec3 x = { 0, 0.1, 0 };
    int i;
    glBegin(GL_LINES);
  ...
  */
  /*
void drawmodel(struct ov_model *model)
{
